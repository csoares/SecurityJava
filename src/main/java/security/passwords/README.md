...
```mermaid
flowchart LR
    subgraph Rates ["⚡ Cracking Speed Comparison (1 GPU)"]
        R1["SHA-256  →  10,000,000,000 hashes/sec  (10 billion)"]
        R2["PBKDF2 310k → 10,000 hashes/sec  (ten thousand)"]
        R3["bcrypt cost=12 → 5,000 hashes/sec"]
        R4["Argon2id → 500 hashes/sec  (memory-hard)"]
        R1 --- R2 --- R3 --- R4
    end
```
