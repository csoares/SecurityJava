...
flowchart LR
    subgraph Comparison ["Symmetric vs Asymmetric"]
        SYM["🔒 Symmetric (AES)<br/>✅ Fast — GB/sec<br/>✅ Simple<br/>❌ Key sharing problem<br/>→ Use for bulk data"]
        ASYM["🔑 Asymmetric (RSA)<br/>✅ No key sharing problem<br/>✅ Public key safe to share<br/>❌ Slow — KB/sec<br/>❌ Limited payload size<br/>→ Use for key exchange only"]
    end
```
