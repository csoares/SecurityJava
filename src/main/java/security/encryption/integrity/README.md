...
flowchart LR
    subgraph Avalanche ["⚡ Avalanche Effect — tiny change, huge difference"]
        ...
        D1 -->|"SHA-256"| H1
        D2 -->|"SHA-256"| H2
        DIFF["Completely different hash!\nNo way to tell how much the input changed."]
        H1 & H2 --> DIFF
    end
```
...
flowchart LR
    ...
    RECV -->|"recompute SHA-256(data)"| CMP{"Hashes\nequal?"}
...
flowchart TD
    subgraph Limits ["⚠️ Limitations of Plain Hashing"]
        ...
        L4["→ Use HMAC to add a secret key\n→ Use Digital Signatures to add identity"]
    end
```
...
flowchart LR
    subgraph Sign ["Signing"]
        ...
        HASH -->|"RSA-encrypt\nwith PRIVATE key"| SIG["Signature\n(256 bytes)"]
    end

    subgraph Verify ["Verification"]
        SIG2["Signature"] -->|"RSA-decrypt\nwith PUBLIC key"| HASH2["Original Hash"]
...
flowchart TD
    ...
    G4["❌ Does NOT prove the key holder's real-world identity\n→ Need a Certificate Authority (PKI) to bind key to identity"]
```
...
flowchart LR
    subgraph Sign ["✍️ Signing  (Sender)"]
        FILE["📄 document.txt"]
        HASH["SHA-256 Hash\n(32 bytes — fixed size regardless of file size)"]
        SIG["🔏 signature.sig\n(256 bytes)"]
        FILE -->|"computeSHA256()"| HASH
        HASH -->|"RSA sign\n(private key)"| SIG
        WHY["Why hash first?\nRSA can only encrypt ~245 bytes.\nHashing reduces any file to 32 bytes."]
    end
```
...
flowchart LR
    subgraph Verify ["🔍 Verification  (Receiver)"]
        ...
        HASH2 --> CMP
        ORIG --> CMP
        CMP -->|"yes"| OK["✅ Authentic\n& Unmodified"]
        CMP -->|"no"| FAIL["❌ Tampered\nor wrong key"]
    end
```
