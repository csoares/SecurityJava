...
flowchart TD
    subgraph Encrypt
        T_Encrypt["🔒 Encryption  (key = shift 3)"]
        T_Encrypt ~~~ PL
    ...
    subgraph Decrypt
        T_Decrypt["🔓 Decryption  (same key, reversed)"]
        T_Decrypt ~~~ CT2
...
flowchart TD
    subgraph Alphabet
        T_Alphabet["Alphabet shift — shift = 3, wraps around at Z"]
        T_Alphabet ~~~ A1
...
flowchart TD
    subgraph BruteForce
        T_BruteForce["⚠️ Brute Force Attack — only 25 keys to try"]
        T_BruteForce ~~~ BF1
    ...
    subgraph FreqAnalysis
        T_FreqAnalysis["📊 Frequency Analysis (no key needed)"]
        T_FreqAnalysis ~~~ FA1
...
flowchart TD
    subgraph KeyRepeat
        T_KeyRepeat["🔑 Keyword repeats to match plaintext length"]
        T_KeyRepeat ~~~ KR1
    ...
    subgraph PerLetter
        T_PerLetter["🔤 Each letter uses a different Caesar shift"]
        T_PerLetter ~~~ L1
        L1["H + shift 10 = R"]
        L2["E + shift  4 = I"]
        L3["L + shift 24 = J"]
...
flowchart TD
    subgraph Kasiski
        T_Kasiski["⚠️ Kasiski Examination"]
        T_Kasiski ~~~ K1
